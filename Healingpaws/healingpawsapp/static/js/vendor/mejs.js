...
$(function () {
    ...
    $('#show_tbody').on('click','.edit', function () {
        ...
        addEnter = false;
        $(this).parents('tr').addClass('has_case');$(function () {
...
var methods = {
    ...
    editHandle: function (the_index) {
        ...
        for (var j=0;j<tarInp.length+1;j++) {
            tarInp.eq(j).val(nowConArr[j])
        }
        for (var p=0;p<tarSel.length+1;p++) {
            var the_p = p+tarInp.length;
            tarSel.eq(p).val(nowConArr[the_p]);
        }
    ...
    setStr: function () {
        ...
        tdStr = '';
        for (var a=0; a<tarInp.length+1; a++) {
            tdStr+= '<td>' + tarInp.eq(a).val() + '</td>'
        }
        for (var b=0; b<tarSel.length+1; b++) {
            tdStr+= '<td>' + tarSel.eq(b).val() + '</td>'
        }
